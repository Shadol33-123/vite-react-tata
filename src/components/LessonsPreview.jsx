...
export default function LessonsPreview() {
  // Unimos las lecciones de todos los niveles en un solo array
  const allLessons = [
    ...lessons.basico,
    ...lessons.medio,
    ...lessons.avanzado
  ...
  // Seleccionamos solo las primeras 3 para el inicio
  const top = allLessons.slice(0, 3);
    ...
    <div>
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {top.map((item) => (
          <article
            key={item.id}
            className="card p-5 flex flex-col hover:-translate-y-0.5"
          >
      ...
      <div className="mt-6">
        <Link to="/lecciones" className="btn btn-ghost">
          Ver todas
        </Link>
      </div>
    </div>
